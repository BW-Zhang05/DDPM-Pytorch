...
def _build_model(*, image_size, in_ch, model_var_type, dropout):
...
def train(
    ...
    model = _build_model(image_size=image_size, in_ch=3, model_var_type=model_var_type, dropout=dropout).to(device) # U-NET
    ...
    while step < max_steps:
        ...
        losses = diffusion.training_losses(lambda _x, _t: model(_x, _t), x_start=x, t=t)
        loss = losses.mean()
        ...
        if step % sample_interval == 0:
            ...
            with torch.no_grad():
                ...
                samples = diffusion.p_sample_loop(lambda _x, _t: model(_x, _t), shape=sample_shape, device=device)
...
def sample(
    ...
    model_var_type = config.get('model_var_type', model_var_type)
    ...
    model = _build_model(image_size=image_size, in_ch=3, model_var_type=model_var_type, dropout=0.0).to(device)
    model.load_state_dict(ckpt['model'])
    model.eval()
